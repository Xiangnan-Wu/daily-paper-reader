...
tldr: 本研究针对现有视觉-语言-动作（VLA）模型仅关注预测未来绝对状态而忽视演变过程的问题，提出了ΔVLA框架。该框架通过建模相对于当前世界知识先验的变化量来指导动作生成。核心贡献包括：利用PWKE提取关键空间与语义先验，通过LWVQ在离散潜空间中量化知识演变，以及设计CV-Atten减少建模干扰。实验证明，ΔVLA在模拟和真实机器人任务中均取得SOTA性能，且具备更高的效率。
selection_source: fresh_fetch
motivation: 现有的VLA模型侧重于预测绝对的未来世界状态，而忽视了对指导动作生成的关键演变过程（变化量）的推理。
method: 提出一种基于先验引导的框架，通过提取世界知识先验并量化其在动作下的潜空间变化量来辅助决策。
result: 在多个模拟基准和真实世界机器人操控任务中，该方法在性能和效率上均超越了现有的先进模型。
conclusion: 显式建模世界知识的变化量而非完整状态，是提升VLA模型感知、推理与控制集成能力的有效途径。
---
