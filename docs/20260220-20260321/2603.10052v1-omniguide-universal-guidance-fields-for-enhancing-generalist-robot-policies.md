...
tldr: OmniGuide是一个旨在提升通用机器人策略（VLA）在复杂任务中表现的通用框架。针对VLA模型在空间理解、语义推理及精准操作方面的局限，该方法通过将3D基础模型、VLM和人体姿态模型等外部引导源转化为3D空间中的可微能量函数（吸引子与排斥子），从而优化动作采样过程。实验表明，OmniGuide显著提高了现有SOTA策略在仿真和现实环境中的成功率与安全性，展现了极强的通用性与互补性。
selection_source: fresh_fetch
motivation: 现有的视觉-语言-动作（VLA）模型在处理涉及复杂空间语义、杂乱环境或高精度要求的任务时性能受限。
method: 提出一种将多种外部引导源统一表示为3D空间中可微能量场的框架，通过定义吸引子和排斥子来引导VLA模型的动作采样。
result: 在多项仿真和真实世界实验中，OmniGuide显著提升了π0.5和GR00T等前沿通用策略的成功率及安全性。
conclusion: OmniGuide证明了通过统一框架整合互补的外部引导源，可以有效增强通用机器人策略应对复杂挑战性任务的能力。
---
