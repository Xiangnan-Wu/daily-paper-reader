...
tldr: DAM-VLA是一个基于动态动作模型的视觉-语言-动作框架，旨在解决机器人操作中通用任务适应性与精细操作精度之间的矛盾。它将预训练VLM的推理能力与专门用于机械臂和夹持器控制的扩散动作模型相结合，通过动作路由、特征融合和双尺度动作加权机制，实现了从粗略移动到精确操作的无缝衔接。在模拟和真实环境的复杂任务中表现优异，展现了强大的泛化能力。
selection_source: fresh_fetch
motivation: 现有的VLA框架在处理需要高精度的复杂操作任务时，难以兼顾通用任务的适应性与精细控制的专业性。
method: 该框架集成了VLM推理与扩散动作模型，并引入动作路由、动态特征融合及双尺度动作加权机制来协调机械臂与夹持器的运动。
result: 在SIMPLER、FurnitureBench及真实世界评估中，DAM-VLA在长程和接触密集型任务上的成功率均优于当前最先进的方法。
conclusion: 通过将高层认知推理与底层动态动作模型相结合，DAM-VLA显著提升了机器人在动态环境下的复杂操作性能和鲁棒性。
---
