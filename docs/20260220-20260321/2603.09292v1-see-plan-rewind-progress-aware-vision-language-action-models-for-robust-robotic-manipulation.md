...
tldr: "针对机器人操作中缺乏进度感知导致难以应对失败的问题，本文提出 SPR（See, Plan, Rewind）框架。该框架将语言指令动态转化为空间子目标序列，通过“观察当前状态、规划轨迹、失败时回溯”的闭环机制实现鲁棒操作。SPR 无需额外训练数据即可实现错误纠正，在 LIBERO 和 LIBERO-Plus 基准测试中表现优异，显著提升了模型在处理未知指令和初始状态时的分布外鲁棒性。"
...
conclusion: SPR 证明了进度感知能力在提升视觉-语言-动作模型（VLA）的泛化性和分布外鲁棒性方面的关键作用。
---
