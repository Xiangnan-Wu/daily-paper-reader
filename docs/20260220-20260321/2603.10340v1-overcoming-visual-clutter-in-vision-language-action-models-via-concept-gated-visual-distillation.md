...
## 摘要
视觉-语言-动作（VLA）模型展现了令人印象深刻的零样本泛化能力，但在杂乱环境中经常面临“精度-推理差距”（Precision-Reasoning Gap）。这种失效是由背景引起的特征稀释驱动的，其中高频语义噪声破坏了精确操作所需的几何接地（geometric grounding）。为了弥补这一差距，我们提出了概念门控视觉蒸馏（CGVD），这是一种无需训练且与模型无关的推理框架，旨在稳定 VLA 策略。CGVD 通过将指令解析为安全集和干扰集，并利用结合了交叉验证与空间消歧的双层目标细化过程，来显式惩罚假阳性并隔离真实的操作目标。随后，我们通过基于傅里叶的修复技术处理场景，生成干净的观测图像，在保留关键空间几何和视觉本体感受的同时，主动抑制语义干扰项。在高度杂乱的操作任务中进行的广泛评估表明，CGVD 能够防止性能崩溃。在具有密集语义干扰项的环境中，我们的方法显著优于最先进的基准模型，成功率从基准的 43.0% 提升至 77.5%。通过强制执行严格的属性遵循，CGVD 将推理时视觉蒸馏确立为在杂乱环境中实现鲁棒机器人操作的关键前提。
