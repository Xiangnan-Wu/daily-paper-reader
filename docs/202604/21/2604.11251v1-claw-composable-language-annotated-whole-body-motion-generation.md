...
tldr: 针对人形机器人全身运动控制中语言标注数据匮乏的问题，本文提出了CLAW，一个基于Web的交互式数据生成流水线。该系统通过组合运动基元，结合MuJoCo物理仿真和自动模板标注，为Unitree G1机器人生成物理可行且带自然语言描述的运动轨迹，为大规模人形机器人学习提供了高效、可扩展的数据生成工具。
selection_source: fresh_fetch
motivation: 现有的动捕数据成本高且多样性有限，而纯运动生成模型往往缺乏物理可行性，难以满足人形机器人学习的需求。
method: 开发了一个基于Web的系统，通过组合运动基元并利用低级控制器在MuJoCo中生成物理落地轨迹，同时配合模板引擎自动生成多风格语言标注。
result: 实现了一个支持实时控制和批量编辑的开源平台，能够为Unitree G1机器人高效产出高质量、物理一致的语言-运动配对数据。
conclusion: CLAW为人形机器人领域提供了一种可扩展的语言标注运动数据生成方案，有效降低了数据获取门槛并提升了数据质量。
---
