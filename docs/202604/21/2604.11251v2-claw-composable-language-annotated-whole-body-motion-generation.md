...
tldr: CLAW是一个为Unitree G1人形机器人设计的交互式Web流水线，旨在解决高质量语言-全身运动配对数据稀缺的问题。它通过将运动模式视为可组合的构建块，结合MuJoCo物理仿真确保运动的物理可行性，并利用模板化引擎自动生成多风格的自然语言标注。该系统支持实时键盘控制和时间轴编辑，为人形机器人学习提供了可扩展且低成本的数据生成方案。
selection_source: fresh_fetch
motivation: 现有的动捕数据成本高且多样性有限，而纯运动生成模型往往缺乏物理可行性，阻碍了语言驱动的人形机器人控制器的训练。
method: 开发了一个基于Web的系统，通过组合式运动规划器生成参考轨迹，并在MuJoCo中利用低级控制器实现物理接地，同时自动生成对应的语言描述。
result: 成功实现了一个能够高效生成大规模、物理一致且带有丰富语言标注的全身运动数据集的开源工具。
conclusion: CLAW为人形机器人领域提供了一种可扩展的语言-运动数据生成框架，有效弥补了高质量具身智能训练数据的短板。
---

## 摘要
为人形机器人训练以语言为条件的全身控制器，需要将运动轨迹与自然语言描述配对的大规模数据集。现有的基于运动捕捉的方法成本高昂且多样性有限，而文本到运动生成模型产生的纯运动学输出无法保证物理可行性。因此，我们提出了 CLAW，这是一个基于 Web 的交互式流水线，用于为宇树 G1 人形机器人可扩展地生成语言标注的全身运动数据。CLAW 将运动学规划器的运动模式视为可组合的构建模块，每个模块由移动、朝向、速度、骨盆高度和持续时间进行参数化，并提供两个基于浏览器的界面——实时键盘模式和基于时间轴的序列编辑器——用于探索性和批量数据采集。底层全身控制器在 MuJoCo 仿真中跟踪规划器的运动学参考，产生以 50Hz 记录的具有物理基础的轨迹。同时，一个基于确定性模板的标注引擎为每个片段和完整轨迹生成多种语体风格的多样化自然语言描述。我们将该系统开源，以支持为人形机器人学习可扩展地生成语言-运动配对数据。
