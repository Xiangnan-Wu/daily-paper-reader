...
## 摘要
由于其简单性和可重复性，体素网格强化学习被广泛应用于冗余机械臂的路径规划。然而，在七自由度机械臂上通过逐点数值逆运动学直接执行，往往会导致步长抖动、关节突变以及在奇异构型附近的不稳定性。本研究提出了一种在不修改离散规划器本身的情况下，衔接离散规划与连续执行的框架。在规划侧，引入了步长归一化的26邻域笛卡尔动作和几何平局打破机制，以抑制不必要的转向并消除步长振荡。在执行侧，实现了一个任务优先级阻尼最小二乘（TP-DLS）逆运动学层。该层将末端执行器位置视为主要任务，而将姿态和关节中心化作为投影到零空间的从属任务处理，并结合了置信域裁剪和关节速度约束。在随机稀疏、中等和密集环境下的七自由度机械臂上，该衔接框架将密集场景中的规划成功率从约0.58提高到1.00，将代表性路径长度从约1.53米缩短至1.10米，并在保持末端执行器误差低于1毫米的同时，将峰值关节加速度降低了一个数量级以上，显著提升了冗余机械臂上基于体素的强化学习路径的连续执行质量。
