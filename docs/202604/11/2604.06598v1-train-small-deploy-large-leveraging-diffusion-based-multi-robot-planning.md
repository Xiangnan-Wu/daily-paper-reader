...
tags: ["query:robot"]
score: 6.0
evidence: 基于扩散模型的机器人路径规划器
tldr: 针对多机器人路径规划中学习模型难以扩展至更多机器人且训练成本高的问题，本文提出一种基于扩散模型的规划器。该方法通过共享扩散模型结合智能体间注意力机制和时间卷积，实现了“小规模训练、大规模部署”的范式。实验证明，该方法在训练少量智能体后，能有效泛化至更多智能体的复杂场景，在准确性和扩展性上优于现有强化学习和启发式方法。
selection_source: fresh_fetch
motivation: 现有的多机器人路径规划方法在面对部署时机器人数量增加时往往失效，且大规模训练成本极高。
method: 提出一种基于扩散模型的规划器，利用共享模型、智能体间注意力机制和时间卷积处理动态变化的机器人数量。
result: 该方法在仅使用少量智能体训练的情况下，能够成功泛化到更大规模的机器人群体，并保持良好的规划精度。
conclusion: “小规模训练、大规模部署”的范式为解决多机器人系统的可扩展性和泛化性挑战提供了一种高效且稳健的方案。
---

## 摘要
基于学习的多机器人路径规划方法在扩展性或对变化的泛化能力方面存在困难，特别是部署过程中机器人数量的变化。大多数现有方法是在固定数量的机器人上训练的，虽然在测试时可能容许机器人数量减少，但在数量增加时通常会失效。此外，针对更多数量的智能体训练此类方法既耗时又耗费计算资源。然而，解析方法在计算扩展性或处理环境动态变化方面也面临挑战。在这项工作中，我们提出利用一种基于扩散模型的规划器，该规划器能够处理动态变化的智能体数量。我们的方法在有限数量的智能体上进行训练，并在部署期间能有效地泛化到更大规模的智能体群体。结果表明，将单个共享的基于扩散模型的规划器与专门的智能体间注意力计算和时间卷积相结合，可以实现具有良好准确性的“小规模训练，大规模部署”范式。我们在多种场景下验证了我们的方法，并将其性能与现有的多智能体强化学习技术以及基于启发式控制的方法进行了比较。
