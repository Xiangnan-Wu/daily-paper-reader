...
tldr: 针对机器人视觉运动策略在异构硬件上部署难、优化慢的问题，本文提出DC-QFA框架。该框架通过训练一个包含多种架构和混合精度位宽的超网络，并结合硬件感知的查找表进行正则化，实现了针对不同设备的快速子网搜索。此外，引入多步在线蒸馏以保证低精度下的长程操作稳定性。实验证明该方法在多种模型上实现了2-3倍加速且性能几乎无损。
selection_source: fresh_fetch
motivation: 现有的机器人策略优化方法通常针对特定设备且耗时较长，难以适应多样化的硬件约束。
method: 提出DC-QFA框架，利用包含架构与量化位宽的超网络进行一次性训练，并通过硬件感知搜索和多步在线蒸馏实现高效部署。
result: 在多种主流策略模型上实现了2-3倍的推理加速，且在仿真和真实机器人任务中均保持了极高的成功率。
conclusion: DC-QFA为异构机器人硬件上的高效策略部署提供了一种通用且快速的解决方案，有效平衡了计算效率与操作精度。
---
