...
title_zh: ReplicateAnyScene：通过文本-视觉-空间对齐实现零样本视频到三维场景组合
...
tldr: ReplicateAnyScene是一个零样本视频到3D场景组合框架，旨在解决现有方法依赖人工提示、受限于简单场景等问题。该方法通过五阶段级联流程，从视觉基础模型中提取并对齐文本、视觉和空间维度的通用先验，将随意拍摄的视频自动转化为结构化的3D场景。此外，研究者还提出了C3DR基准测试集，实验证明该方法在生成高质量组合3D场景方面优于现有基准。
selection_source: fresh_fetch
motivation: 现有组合式3D重建方法过度依赖人工干预且难以处理复杂场景，限制了其在空间智能和具身AI中的应用。
method: 提出一种五阶段级联框架，通过文本、视觉和空间维度的对齐，将视觉基础模型的通用先验整合进结构化3D表示中。
result: 在提出的C3DR基准测试及广泛实验中，该方法在生成高质量、语义一致且物理合理的组合3D场景方面表现显著优于现有基准。
conclusion: ReplicateAnyScene实现了全自动、零样本的视频到3D场景转换，为空间智能领域提供了高效的结构化场景构建方案。
---

## 摘要
人类具有从视频观测中快速感知和分割物体，甚至在脑海中将其组装成结构化三维场景的先天能力。复制这种被称为组合式三维重建（compositional 3D reconstruction）的能力，对于空间智能（Spatial Intelligence）和具身智能（Embodied AI）的发展至关重要。然而，由于跨模态信息整合不足，现有方法难以实现实际部署，导致其依赖手动物体提示、依赖辅助视觉输入，并因训练偏差而局限于过于简单的场景。为了解决这些局限性，我们提出了 ReplicateAnyScene，这是一个能够将随意拍摄的视频全自动且零样本（zero-shot）地转换为组合式三维场景的框架。具体而言，我们的流水线包含一个五阶段级联过程，用于从视觉基础模型中提取并在文本、视觉和空间维度上结构化地对齐通用先验，将其落地（grounding）为结构化三维表示，并确保所构建场景的语义一致性和物理合理性。为了促进对该任务更全面的评估，我们进一步引入了 C3DR 基准测试，从多个维度评估重建质量。大量实验表明，我们的方法在生成高质量组合式三维场景方面优于现有的基准方法。
