...
tags: ["query:robot"]
score: 7.0
evidence: 用于机器人抓取演示的生成模型
tldr: 针对开放环境下功能性抓取的通用机器人构建难题，本文提出 GraspDreamer。该方法利用在大规模互联网数据上预训练的视觉生成模型（VGM）合成人类演示视频，从而提取人类与物理世界交互的先验知识。通过将这些先验与特定机器人的动作优化相结合，实现了无需大规模人工采集数据的零样本功能性抓取，在数据效率和泛化性上显著优于传统方法，并支持真实机器人部署。
selection_source: fresh_fetch
motivation: 传统方法在实现开放环境下的功能性抓取时，面临物体多样性高且人工采集大规模交互数据成本极高的问题。
method: 利用视觉生成模型合成人类操作演示，并结合特定机器人的动作优化，将互联网规模的人类行为先验转化为抓取策略。
result: 实验证明 GraspDreamer 在多个基准测试和真实机器人任务中展现出卓越的泛化能力，并能有效支持下游操控任务的学习。
conclusion: 该研究验证了利用生成式模型合成虚拟演示是实现低成本、高泛化机器人功能性抓取的一种极具潜力的新范式。
---

## 摘要
由于物体和任务的巨大多样性，构建能够在日常开放世界环境中执行功能性抓取的通用机器人仍然是一个重大挑战。现有方法要么受限于狭窄的物体/任务集，要么依赖于极具挑战性的大规模数据收集来捕捉现实世界的变异性。在这项工作中，我们提出了一种替代方法 GraspDreamer，该方法利用视觉生成模型（VGM）（如视频生成模型）合成的人类演示，在无需劳动密集型数据收集的情况下实现零样本功能性抓取。其核心思想是，在互联网规模的人类数据上预训练的 VGM 隐式编码了关于人类如何与物理世界互动的泛化先验，这些先验可以与特定具身的动作优化相结合，从而以极小的代价实现功能性抓取。在不同机器人手上的公开基准测试中进行的大量实验表明，与之前的方法相比，GraspDreamer 具有卓越的数据效率和泛化性能。现实世界的评估进一步验证了其在真实机器人上的有效性。此外，我们展示了 GraspDreamer 可以（1）自然地扩展到下游操作任务，以及（2）生成数据以支持视觉运动策略学习。
