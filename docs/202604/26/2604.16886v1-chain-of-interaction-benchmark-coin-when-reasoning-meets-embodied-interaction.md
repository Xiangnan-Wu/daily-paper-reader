...
tldr: 针对通用具身智能体在长程任务中缺乏交互式推理能力的问题，本文提出了COIN基准测试。该基准包含50个日常交互任务、基础动作集及组合任务，旨在评估智能体在部分可观测环境下的因果推理与执行能力。研究团队还开发了低成本AR遥操作系统并构建了包含1000个演示的原始数据集。通过对CodeAsPolicy、VLA等主流方法的系统评估，揭示了当前模型在视觉理解与动作执行之间存在的显著差距，为具身智能的交互推理研究提供了重要参考。
selection_source: fresh_fetch
motivation: 现有基准测试无法系统评估具身智能体在处理长程任务时所需的交互式、因果依赖的推理能力。
method: 提出了包含COIN-50、Primitive和Composition三个层级的基准，并开发了低成本AR遥操作系统以收集1000条原始任务演示数据。
result: 实验评估显示，CodeAsPolicy、VLA和H-VLA等主流方法在处理复杂的交互式推理任务时表现不佳。
conclusion: 当前具身智能模型在视觉理解与电机执行之间存在巨大鸿沟，难以有效应对需要持续环境交互和计划更新的挑战。
---

## 摘要
通用具身智能体必须执行交互式的、具有因果依赖的推理，通过不断与环境交互、获取信息并更新计划来解决长程任务，才能被应用于现实场景。例如，从柜子中取出苹果可能需要先打开多扇门和抽屉，苹果才变得可见且可触及，这要求在部分可观测性下进行序列化交互。然而，现有的基准测试未能系统地评估这一核心能力。我们推出了 COIN，这是一个旨在通过三项关键贡献评估现实机器人操作中交互式推理能力的基准。首先，我们构建了 COIN-50，包含 50 个日常场景下的交互式任务；并创建了因果依赖任务所需的 COIN-Primitive，以及具有中期复杂度、用于技能学习和泛化评估的 COIN-Composition。其次，我们开发了一套低成本的移动增强现实（AR）遥操作系统，并收集了 COIN-Primitive 数据集，每个元任务包含 50 次演示（总计 1,000 次）。第三，我们制定了关于执行稳定性和泛化鲁棒性的系统评估指标，用于评估 CodeAsPolicy、VLA 以及语言条件的 H-VLA 方法。我们的全面评估揭示了当前方法的关键局限性：由于视觉理解与动作执行之间存在显著鸿沟，模型在处理交互式推理任务时表现挣扎。我们对这些局限性进行了细粒度的分析。
