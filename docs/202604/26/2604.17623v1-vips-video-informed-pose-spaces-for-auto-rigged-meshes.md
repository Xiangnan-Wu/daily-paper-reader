...
## 摘要
运动学绑定为 3D 网格的关节运动提供了结构化接口，但它们缺乏对给定资产合理关节配置流形的内在表示。如果没有这样的姿态空间，对原始绑定参数的随机采样或手动操作往往会导致语义或几何冲突，例如解剖学上的过度伸展和非物理的自相交。我们提出了视频引导姿态空间（ViPS），这是一个前馈框架，通过从预训练的视频扩散模型中蒸馏运动先验，来发现自动绑定网格有效关节运动的潜分布。与依赖稀缺的艺术家创作 4D 数据集的现有方法不同，ViPS 将生成式视频先验转化为给定绑定参数化上的通用分布。应用于蒙皮网格的可微几何验证器在不需要手动正则化项的情况下，强制执行特定资产的有效性。我们的模型学习了一个平滑、紧凑且可控的姿态空间，支持多样化采样、用于逆运动学的流形投影以及用于关键帧生成的时序相干轨迹。此外，蒸馏出的 3D 姿态样本可作为引导视频扩散的精确语义代理，有效地闭合了生成式 2D 先验与结构化 3D 运动控制之间的环路。我们的评估表明，仅在视频先验上训练的 ViPS 在合理性和多样性方面均达到了在合成艺术家创作 4D 数据上训练的最先进方法的性能。最重要的是，作为一个通用模型，ViPS 展示了对分布外物种和未见骨骼拓扑的鲁棒零样本泛化能力。
