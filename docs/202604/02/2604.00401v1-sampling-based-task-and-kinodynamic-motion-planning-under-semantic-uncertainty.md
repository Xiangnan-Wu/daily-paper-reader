...
## 摘要
本文探讨了不确定环境下的集成任务与动力学运动规划问题。我们考虑一个具有非线性动力学的机器人，其任务由有限轨迹上的线性时序逻辑（LTLf）规范定义，并在部分可观测的环境中运行。具体而言，不确定性体现在环境的语义标签中。我们展示了如何将该问题建模为部分可观测随机混合系统，以捕捉机器人动力学、LTLf 任务以及环境状态变量的不确定性。我们提出了一种随时（anytime）算法，该算法利用了混合系统的结构，并结合了决策技术与基于采样的运动规划的有效性。我们证明了该算法的可靠性与渐近最优性。实验结果表明了该算法在不确定环境中的有效性，且其表现始终优于基准方法。
