...
tldr: 针对视频外扩中存在的时空不一致性问题，本文提出Seen-to-Scene框架，将传播与生成范式相结合。该方法利用微调的光流补全网络重建运动场，并引入参考引导的潜空间传播技术，以确保源内容在帧间的高效传递。实验证明，该方法在保持时空连贯性和视觉真实感方面显著优于现有技术，且具备更高的推理效率。
selection_source: fresh_fetch
motivation: 现有的基于扩散模型的视频外扩方法由于缺乏显式时空建模，在处理动态场景和大范围外扩时容易出现帧内和帧间的不一致性。
method: 提出一种统一传播与生成的框架，通过端到端微调的光流补全网络重建运动场，并结合参考引导的潜空间传播来增强内容的一致性。
result: 实验结果表明，该方法在时空连贯性和视觉真实感上均超越了现有最先进的方法，且无需针对特定输入进行繁琐的适配。
