...
tags: ["query:robot"]
score: 7.0
evidence: 大语言模型引导的机器人抓取规划
tldr: 针对面向任务的抓取（TOG）中语义识别不稳定和泛化性差的问题，本文提出一种以几何为中心的通用策略。该方法利用大语言模型构建物体-部件-任务本体以选择功能部件，通过采样几何分析从点云中识别目标部件，并结合相似性匹配框架，利用已知物体的先验知识指导未知目标的抓取规划。实验证明该方法在功能部件识别和抓取生成方面具有高准确性，且能有效泛化至新类别物体。
selection_source: fresh_fetch
motivation: 现有的基于视觉语言模型的任务导向抓取方法在部件识别和推理上存在不稳定性，难以应对多样化的物体和任务。
method: 结合大语言模型引导的本体库进行部件选择，利用几何采样分析识别点云部件，并通过相似性匹配框架借鉴已知物体的抓取经验。
result: 实验验证了该方法在功能部件选择、识别及抓取生成上的高准确率，并展示了其对新类别物体的强泛化能力。
conclusion: 该研究通过融合大语言模型的常识推理与几何相似性匹配，为实现更通用、稳健的任务导向机器人抓取提供了有效方案。
---

## 摘要
面向任务的抓取（TOG）比简单的物体抓取更具挑战性，因为它需要精确识别物体部件并仔细选择抓取区域，以确保操作的有效性和鲁棒性。虽然最近的方法通过训练大规模视觉语言模型来整合部件级物体分割与任务感知抓取规划，但它们在部件识别和抓取推理方面的不稳定性限制了其在不同物体和任务中的泛化能力。为了解决这一问题，我们提出了一种新颖的、以几何为中心的可泛化 TOG 策略，该策略不依赖于视觉识别的语义特征，从而有效克服了基于模型的方法对视角的敏感性。我们的主要贡献包括：1）一种利用大语言模型（LLM）构建的“物体-部件-任务”本体，用于根据直观的人类指令选择功能部件；2）一种基于采样的几何分析方法，通过结合多种点分布和距离度量，从观测到的点云中识别所选物体部件；3）一种用于模仿抓取规划的相似性匹配框架，利用具有预先存在的分割和抓取知识的相似已知物体作为参考，来指导未知目标的规划。我们通过真实世界实验验证了该方法在功能部件选择、识别和抓取生成方面的高准确性。此外，我们通过扩展现有的本体知识，展示了该方法对新类别物体的泛化能力，体现了其对广泛物体和任务的适应性。
