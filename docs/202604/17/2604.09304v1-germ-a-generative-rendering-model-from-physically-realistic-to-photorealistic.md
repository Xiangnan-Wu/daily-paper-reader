...
tldr: 本研究针对基于物理渲染（PBR）与真实感渲染（PRR）之间的差距，提出了首个多模态生成式渲染模型 GeRM。通过引入分布传输向量场（DTV Field）理论，GeRM 结合了 G-buffer、文本提示和渐进式增量注入技术，实现了从物理保真度到感知真实感的平滑过渡。此外，研究者利用多智能体 VLM 框架构建了 P2P-50K 数据集，有效解决了传统渲染在模型复杂度和生成模型在可控性之间的矛盾。
selection_source: fresh_fetch
motivation: 传统的基于物理渲染受限于现实世界数字模型的缺失难以达到真正的照片级真实感，而生成模型又往往牺牲了可控性和几何一致性。
method: 提出 GeRM 模型，通过多条件 ControlNet 学习分布传输向量场，并利用多智能体 VLM 框架构建的 P2P-50K 成对数据集引导 PBR 到 PRR 的转换。
result: GeRM 能够整合物理属性与文本提示，生成既符合几何约束又具备高度真实感的图像，实现了物理保真度与感知真实感之间的灵活平衡。
conclusion: 该研究通过统一 PBR 与 PRR，为弥补物理模拟与生成式真实感之间的鸿沟提供了一种高效且可控的新范式。
---

## 摘要
几十年来，基于物理的渲染（PBR）一直是合成照相写实图像的基础，因此有时被粗略地称为照相写实渲染（PRR）。虽然 PBR 确实是保证物理真实性的光线传输数学模拟，但照相写实性还额外依赖于现实世界几何和外观的真实数字模型，这在 PBR 到 PRR（P2P）之间留下了一个几乎未被探索的鸿沟。因此，通往照相写实之路面临着一个关键困境：PRR 的显式模拟受限于现实世界中难以获取的真实数字模型，而隐式生成模型则牺牲了可控性和几何一致性。基于这一洞察，本文提出了缓解 P2P 鸿沟的问题、数据和方法，并随后推出了首个统一 PBR 和 PRR 的多模态生成式渲染模型，命名为 GeRM。GeRM 整合了 G-buffer 等物理属性与文本提示，并通过渐进式增量注入来生成可控的照相写实图像，允许用户在严格的物理忠实度和感知照相写实性之间流畅切换。在技术上，我们将 PBR 和 PRR 图像之间的转换建模为分布转移，并旨在学习一个分布转移向量场（DTV Field）来指导这一过程。为了定义学习目标，我们首先利用多智能体视觉语言模型（VLM）框架构建了一个专家引导的成对 P2P 转移数据集，命名为 P2P-50K，其中数据集中的每个成对样本对应于 DTV Field 中的一个转移向量。随后，我们提出了一个多条件 ControlNet 来学习 DTV Field，该网络在 G-buffer、文本提示和增强区域线索的引导下，合成 PBR 图像并将其逐步转换为 PRR 图像。
