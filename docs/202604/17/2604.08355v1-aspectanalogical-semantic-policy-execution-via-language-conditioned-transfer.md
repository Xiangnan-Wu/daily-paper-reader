...
tldr: 针对强化学习智能体在结构相似任务中泛化能力差的问题，本文提出ASPECT框架。该方法放弃了预定义的离散类别，转而利用文本条件的变分自编码器（VAE）和大型语言模型（LLM）。在测试时，LLM作为语义算子，将新任务的观察描述重映射为源任务描述，通过VAE生成与原始训练兼容的想象状态，从而实现策略的零样本迁移，显著提升了智能体在复杂类比任务中的适应性。
...
conclusion: 通过结合LLM的灵活推理能力与生成模型，ASPECT显著提升了强化学习智能体在处理未见过的类比任务时的泛化与迁移能力。
---

## 摘要
强化学习（RL）智能体通常难以将知识泛化到新任务中，即使这些任务在结构上与它们已经掌握的任务相似。尽管最近的方法尝试通过零样本迁移来缓解这一问题，但它们通常受限于预定义的离散类别系统，从而限制了其对新颖或组合式任务变体的适应能力。我们提出了一种更具泛化性的方法，通过文本条件化的变分自编码器（VAE），用自然语言条件化取代了离散隐变量。我们的核心创新在于测试时利用大语言模型（LLM）作为动态的“语义算子”。我们的智能体不再依赖僵化的规则，而是通过查询 LLM 对当前观测的描述进行语义重映射，使其与源任务对齐。这种与源任务对齐的描述作为条件引导 VAE 生成一个与智能体原始训练兼容的想象状态，从而实现策略的直接复用。通过利用 LLM 灵活的推理能力，我们的方法在广泛的复杂且真正新颖的类比任务中实现了零样本迁移，超越了固定类别映射的局限性。代码和视频可在提供的链接中获取。
