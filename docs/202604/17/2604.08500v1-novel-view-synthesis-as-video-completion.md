...
tldr: 本研究将稀疏视角合成（NVS）重新定义为低帧率视频补全任务，旨在利用视频扩散模型中蕴含的隐式多视图一致性知识。针对输入图像无序且稀疏的挑战，提出了 FrameCrafter 框架，通过移除时间位置编码和引入逐帧潜变量编码，使视频模型具备置换不变性。该方法仅需少量监督即可让视频模型适应非时序的视角生成，在多个稀疏视角合成基准上取得了极具竞争力的结果。
selection_source: fresh_fetch
motivation: 现有的图像扩散模型缺乏多视图一致性知识，而视频模型天然具备处理空间相关性的潜力，更适合通过视频补全的方式解决视角合成问题。
method: 开发了 FrameCrafter 框架，通过移除时间位置编码和采用逐帧潜变量编码，将视频扩散模型改造为对输入顺序不敏感的置换不变架构。
result: 实验证明视频模型可以被训练为忽略时间顺序，在稀疏视角合成任务中表现优异，达到了当前主流基准的领先水平。
conclusion: 将 NVS 视为视频补全任务并对视频模型进行置换不变性改造，是实现高质量稀疏视角合成的一种简单且有效的方案。
---

## 摘要
我们利用视频扩散模型解决稀疏新视角合成（NVS）问题；给定场景的 $K$（约 5 张）多视图图像及其相机位姿，我们预测目标相机位姿下的视图。许多先前的方法利用了通过扩散模型编码的生成式图像先验。然而，在单张图像上训练的模型缺乏多视图知识。与此不同，我们认为视频模型已经包含了隐式的多视图知识，因此应该更容易适配于 NVS。我们的核心见解是将稀疏 NVS 建模为低帧率视频补全任务。然而，一个挑战在于稀疏 NVS 是在无序输入集上定义的，这些输入通常过于稀疏，难以形成有意义的顺序，因此模型应当对该输入集的排列具有不变性（invariant）。为此，我们提出了 FrameCrafter，它通过若干架构修改，将视频模型（通常在连贯的帧序列上训练）适配于排列不变的 NVS，这些修改包括逐帧潜变量编码和移除时间位置嵌入。我们的结果表明，只需极少的监督，视频模型就可以很容易地被训练成“忘记”时间，并在稀疏视角 NVS 基准测试中产生具有竞争力的性能。项目页面：https://frame-crafter.github.io/
