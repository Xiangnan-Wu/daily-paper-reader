...
## 摘要
文本驱动动作生成旨在生成与输入文本高度对齐、符合物理规律且具有丰富细粒度细节的 3D 人体动作。尽管最近的方法可以产生复杂且自然的运动，但它们通常仅在单一时间尺度上运行，这限制了语义对齐和时间连贯性。受人类认知系统中复杂动作是分层构思而非单一时间尺度这一事实的启发，我们提出了 MotionHiFlow，这是一个分层流匹配框架，通过构建从低到高时间尺度的流路径来逐步生成动作。低尺度的流捕捉高层语义和粗略的动作结构，而高尺度的流则细化时间细节。为了连接跨尺度的流，我们引入了一种新颖的跨尺度过渡过程，确保了连续性并保持了噪声一致性。此外，通过集成文本-动作扩散 Transformer (Text-Motion Diffusion Transformer) 和拓扑感知动作 VAE (topology-aware Motion VAE)，MotionHiFlow 通过关节感知位置编码和骨骼拓扑显式建模关节间的结构依赖关系，从而实现精确的语义对齐和细粒度的动作细节。在 HumanML3D 和 KIT-ML 基准测试上的广泛实验证明了其达到最先进的性能，消融研究证实了分层设计和关键组件的有效性。代码可在 https://github.com/ai-lh/MotionHiFlow 获取。
