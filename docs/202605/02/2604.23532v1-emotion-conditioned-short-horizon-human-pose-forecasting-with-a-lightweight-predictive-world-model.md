...
title_zh: 基于轻量级预测世界模型的情感条件化短时人体姿态预测
authors: "Jingni Huang, Peter Bloodsworth"
date: 2026-04-26
pdf: "https://arxiv.org/pdf/2604.23532v1"
tags: ["query:robot"]
score: 8.0
evidence: 用于机器人中人体姿态预测的轻量级预测世界模型
tldr: 本研究探讨了情感信号对短期人体姿态预测的影响，提出了一种基于轻量级预测世界模型的框架。该模型结合了面部表情衍生的情感嵌入与姿态关键点，通过可学习的门控机制和双层LSTM架构进行15步自回归预测。实验表明，在情感驱动的运动序列中，标准化的门控融合显著提升了预测精度，且反事实实验证实了情感嵌入作为辅助信号的有效性，为情感感知的交互系统提供了新思路。
selection_source: fresh_fetch
motivation: 现有的姿态预测模型主要依赖几何运动线索，忽视了影响人体运动动力学的情感信号。
method: 提出一种轻量级自回归预测世界模型，利用门控机制融合姿态与情感嵌入，并通过双层LSTM进行滚动预测。
result: 实验发现标准化门控融合能显著提高情感驱动运动的预测准确性，且模型对情感输入的改变具有可测量的敏感性。
conclusion: 将面部表情情感嵌入集成到轻量级预测模型中是实现情感条件化短期姿态预测的可行且有效的方法。
---

## 摘要
短期人体姿态预测在交互系统、辅助机器人以及情感感知人机交互中起着至关重要的作用[1-3]。虽然目前的轨迹预测模型主要依赖于几何运动线索，但它们往往忽略了影响人体运动动力学的潜在情感信号[4-5]。本文研究了源自面部表情的情感嵌入是否能为短期姿态预测提供辅助条件信号。为了在递归预测设置中进一步评估多模态条件化，我们提出了一种轻量级自回归预测世界模型，可执行 15 步滚动姿态预测。该框架通过可学习的门控机制将姿态关键点与情感嵌入相结合，并使用基于双层 LSTM 架构的循环序列模型执行自回归展开预测。实验在两个小规模姿态-情感视频数据集上进行：面部表情变化极小的受控运动序列，以及面部表情变化显著的自然情感驱动运动序列。结果表明，简单的多模态融合并不能持续提高预测精度，而归一化门控融合则显著增强了情感驱动运动序列的性能。此外，反事实扰动实验表明，预测轨迹对多模态输入的变化表现出可衡量的敏感性，这表明面部表情嵌入是作为辅助条件信号而非冗余特征发挥作用的。总之，这些结果表明，将源自面部表情的情感嵌入整合到基于轻量级预测世界模型架构的情感条件化短期姿态预测中是一种可行的方法。
