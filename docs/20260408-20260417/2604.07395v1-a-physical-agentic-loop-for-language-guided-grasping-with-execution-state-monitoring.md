...
## 摘要
遵循语言指令的机器人操作系统的抓取原语通常以单次尝试（single-shot）的方式执行：模型提出动作，机器人执行，而诸如空抓、打滑、卡死、超时或语义错误抓取等失败情况并未以结构化的方式反馈给决策层。受数字工具使用智能体中“智能体循环”（agentic loops）的启发，我们将语言引导的抓取重新表述为一个在具身执行状态上运行的有界具身智能体，其中物理动作会暴露显式的工具状态流。我们引入了一种物理智能体循环，它通过 (i) 基于事件的接口和 (ii) 执行监测层 Watchdog，封装了一个未经修改的学习型操作原语（抓取并提升）。Watchdog 利用接触感知融合和时间稳定技术，将嘈杂的夹爪遥测数据转换为离散的结果标签。这些结果事件（可选择性地结合抓取后的语义验证）由一个确定性的有界策略处理，该策略决定完成、重试或上报给用户进行澄清，从而保证有限次终止。我们在配备手眼 D405 相机的移动操作机上验证了该循环，保持底层抓取模型不变，并评估了涉及视觉歧义、干扰物和诱导执行失败的代表性场景。结果表明，显式的执行状态监测和有界恢复比开环执行实现了更鲁棒且更具可解释性的行为，同时增加了极小的架构开销。
