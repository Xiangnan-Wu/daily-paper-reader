...
method: 提出PhyEdit框架，利用显式几何模拟作为即插即用的3D感知引导，并结合2D-3D联合监督来增强物理准确性。
result: 实验表明，该方法在3D几何精度和操控一致性上均优于现有方法，并提供了RealManip-10K数据集和ManipEval评测基准。
conclusion: 通过引入显式3D几何先验，可以有效提升生成模型在真实世界物体操控任务中的物理一致性和空间控制能力。
---
